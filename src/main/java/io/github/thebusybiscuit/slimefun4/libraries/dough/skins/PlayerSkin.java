...
public class PlayerSkin {
    ...
    /**
     * This returns a {@link PlayerProfile} with this skin applied.
     *
     * @return A {@link PlayerProfile} holding this skin
     */
    public final @Nonnull PlayerProfile toPlayerProfile() {
        return profile.toPlayerProfile();
    }
    ...
    public static @Nonnull CompletableFuture<PlayerSkin> fromPlayerUUID(Plugin plugin, UUID uuid) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            try (InputStreamReader reader = new InputStreamReader(new URL(targetUrl).openStream(), StandardCharsets.UTF_8)) {
                ...
                plugin.getLogger().log(Level.SEVERE, "Malformed sessions url: {0}", targetUrl);
                future.completeExceptionally(e);
            } catch (IOException e) {
                plugin.getLogger().log(Level.SEVERE, "Exception while requesting skin: {0}", targetUrl);
                future.completeExceptionally(e);
            }
        });
